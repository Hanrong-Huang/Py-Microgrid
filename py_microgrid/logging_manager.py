...
def _setup_logger():
    ...
    logger for use throughout HOPP.

    Returns:
        logging.Logger: The root logger from the `logging` module.
    """
    # Create a logger object for py_microgrid
    logger = logging.getLogger(name="py_microgrid")
    ...
    file_name = "hopp_{:%Y-%m-%d-%H_%M_%S}.log".format(datetime.now())
