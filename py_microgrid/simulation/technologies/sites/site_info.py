...
from py_microgrid.simulation.technologies.resource import (
    SolarResource,
    WindResource,
    WaveResource,
    ElectricityPrices
)
from py_microgrid.tools.layout.plot_tools import plot_shape
...
class SiteInfo(BaseClass):
    ...
    desired_schedule: Union[NDArrayFloat, str] = field(default=[])
    ...
    def __attrs_post_init__(self):
        ...
        The following are set in this post init hook:
            ...
            solar_resource (:obj:`py_microgrid.simulation.technologies.resource.SolarResource`): Class containing solar resource data.
            wind_resource (:obj:`py_microgrid.simulation.technologies.resource.WindResource`): Class containing wind resource data.
            wave_resoure (:obj:`py_microgrid.simulation.technologies.resource.WaveResource`): Class containing wave resource data.
            elec_prices (:obj:`py_microgrid.simulation.technologies.resource.ElectricityPrices`): Class containing electricity prices.
        ...
        # Handle both list format (legacy) and CSV file path format (new)
        self.desired_schedule = self._load_desired_schedule(self.desired_schedule)
    ...
    def _load_desired_schedule(self, desired_schedule_input):
        """
        Load desired schedule from either a list (legacy format) or CSV file path (new format).
        
        Args:
            desired_schedule_input: Either a list of numeric values or a string path to a CSV file
            
        Returns:
            NDArray: Array of desired schedule values
        """
        # If it's already a list/array, return as-is (backward compatibility)
        if isinstance(desired_schedule_input, (list, np.ndarray)) and len(desired_schedule_input) > 0:
            return np.array(desired_schedule_input)
        
        # If it's a string, treat it as a file path
        elif isinstance(desired_schedule_input, str) and desired_schedule_input.strip():
            # Handle relative paths - make them relative to the YAML file location if needed
            csv_path = desired_schedule_input.strip()
            
            # If path is not absolute, try to find it relative to common locations
            if not os.path.isabs(csv_path):
                # Try relative to current working directory first
                if not os.path.exists(csv_path):
                    # Try relative to the examples directory (common case)
                    examples_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'examples', csv_path)
                    if os.path.exists(examples_path):
                        csv_path = examples_path
                    else:
                        # Try relative to parallel_simulations directory (for load_data folder)
                        parallel_sim_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'examples', 'parallel_simulations', csv_path)
                        if os.path.exists(parallel_sim_path):
                            csv_path = parallel_sim_path
                        else:
                            # Try relative to py_microgrid root
                            root_path = os.path.join(os.path.dirname(__file__), '..', '..', '..', csv_path)
                            if os.path.exists(root_path):
                                csv_path = root_path
            
            if not os.path.exists(csv_path):
                raise FileNotFoundError(f"Desired schedule CSV file not found: {csv_path}")
            
            try:
                # Load CSV file using pandas
                logger.info(f"Loading desired schedule from CSV file: {csv_path}")
                df = pd.read_csv(csv_path)
                
                # Try to find the data column - look for common column names
                data_column = None
                possible_columns = ['load', 'demand', 'schedule', 'value', 'data', 'load data (MW)', 'load data (mw)', 'load_data']
                
                # If only one column, use it
                if len(df.columns) == 1:
                    data_column = df.columns[0]
                else:
                    # Look for a column with one of the expected names
                    for col in possible_columns:
                        if col in df.columns:
                            data_column = col
                            break
                    
                    # If no named column found, use the first numeric column
                ...
                if data_column is None:
                    raise ValueError(f"Could not identify data column in CSV file: {csv_path}")
                
                # Extract the data and convert to numpy array
                schedule_data = df[data_column].values
                logger.info(f"Loaded {len(schedule_data)} schedule values from column '{data_column}'")
                return schedule_data
                
            except Exception as e:
                raise ValueError(f"Error loading desired schedule from CSV file {csv_path}: {str(e)}")
        
        # If empty or invalid input, return empty array
        else:
            return np.array([])
