...
class PredictiveDemandResponseBatteryDispatch(SimpleBatteryDispatchHeuristic):
    ...
    def __init__(
        ...
        # Initialize fixed dispatch array
        self._fixed_dispatch = None
    ...
    def _predictive_method(
        ...
        for t in range(num_periods):
            ...
            if net_load > 0:
                ...
                fd = discharge_amount / self.maximum_power if self.maximum_power > 0 else 0
            else:
                ...
                if future_deficit and current_soc < self.charge_threshold:
                    ...
                    fd = -charge_amount / self.maximum_power if self.maximum_power > 0 else 0
                else:
                    fd = 0

            # Enforce dispatch limits
            fd = np.clip(fd, self.max_discharge_fraction[t], -self.max_charge_fraction[t])
    ...
    def set_fixed_dispatch(
        ...
        # Apply the heuristic method to enforce limits (from the superclass)
    ...
    def update_soc(self, dispatch_factor: float, soc_prev: float) -> float:
        """
        Update the State of Charge (SOC) based on the dispatch factor.

        Args:
            dispatch_factor (float): Dispatch factor for the current period.
            soc_prev (float): Previous SOC value.

        Returns:
            float: Updated SOC value.
        """
        # Calculate energy change based on dispatch factor
        energy_change = -dispatch_factor * self.maximum_power * self.time_step_hours / self.capacity
        soc_new = soc_prev + energy_change

        # Ensure SOC remains within [0, 1]
