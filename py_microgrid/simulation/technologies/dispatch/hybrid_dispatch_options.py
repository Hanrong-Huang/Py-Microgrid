...
from py_microgrid.simulation.technologies.dispatch.power_storage import (
...
class HybridDispatchOptions:
    ...
    Args:
        dispatch_options (dict): Contains attribute key-value pairs to change default options.
            ...
            - **battery_dispatch** (str, default='simple'): Sets the battery dispatch model to use for dispatch. Options are `('simple', 'heuristic', 'one_cycle_heuristic', 'non_convex_LV', 'convex_LV', 'heuristic_load_following', 'predictive_demand_response')`.
    ...
    def __init__(self, dispatch_options: dict = None):
        ...
        self._battery_dispatch_model_options = {
            ...
            "heuristic_load_following": HeuristicLoadFollowingDispatch,
            "predictive_demand_response": PredictiveDemandResponseBatteryDispatch,
        }
        if self.battery_dispatch in self._battery_dispatch_model_options:
            ...
            if ("heuristic" in self.battery_dispatch or 
                self.battery_dispatch in ["heuristic_load_following", "predictive_demand_response"]):
