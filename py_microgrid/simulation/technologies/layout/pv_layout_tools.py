...
from py_microgrid.simulation.technologies.layout.layout_tools import *
from py_microgrid.simulation.technologies.sites.site_info import SiteInfo
from py_microgrid.simulation.technologies.layout.wind_layout_tools import *
...
def place_solar_strands(max_num_modules: int,
    ...
    strands: list[tuple(int, float, Polygon)] = []
...
def get_flicker_loss_multiplier(flicker_data: Tuple[float, np.ndarray, np.ndarray, np.ndarray],
    ...
    if primary_strands is None and module_points is None:
        ...
        total_power = len(module_points.geoms)
        if total_power == 0:
            return 1
        mode = "points"
    else:
    ...
    for n in range(num_turbines):
        ...
        if mode == "points":
            ...
            if modules.is_empty:
                modules = []
            if isinstance(modules, MultiPoint):
                modules = modules.geoms
