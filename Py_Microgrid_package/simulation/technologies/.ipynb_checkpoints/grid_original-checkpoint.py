...
class Grid(PowerSource):
    ...
    def simulate_grid_connection(
        ...
        if self.site.follow_desired_schedule:
            ...
            # generation profile is the minimum of the actual grid generation (total_gen) and the desired schedule (lifetime_schedule), curtail any excess generation beyond the desired schedule
            self.generation_profile = np.minimum(total_gen, lifetime_schedule) # original version is np.minimum()
